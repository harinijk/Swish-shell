    ...
    while (fgets(cmd, CMD_LEN, stdin) != NULL) {
        ...
        if (strcmp(first_token, "pwd") == 0) {
            ...
            if (tokens.length == 1) {
                ...
                if (environment == NULL) {
                    ...
                    perror("Error in home environment variable");
                    strvec_clear(&tokens);
                    continue;
                }
                if (chdir(environment) != 0) {
                    perror("chdir");
                }
            } else {
                const char *environment = strvec_get(&tokens, 1);
                if (environment == NULL) {
                    fprintf(stderr, "Error no directory argument found at index 1\n");
                    strvec_clear(&tokens);
                    continue;
                }
                if (chdir(environment) != 0) {
        ...
        // Task 6: Wait for all background jobs
            ...
            if (pid < 0) {
                ...
                if (background) {
                    if (job_list_add(&jobs, pid, strvec_get(&tokens, 0), BACKGROUND) != 0) {
                    ...
                    if (tcsetpgrp(STDIN_FILENO, pid) == -1) {
                        perror("tcsetpgrp foreground child");
                    }
                    ...
                    // forked, and
                    //    use WUNTRACED as your third argument to detect if it has stopped from a
                    //    signal
                    ...
                    if (WIFSTOPPED(status)) {
                        if (job_list_add(&jobs, pid, strvec_get(&tokens, 0), STOPPED) != 0) {
                            perror("error adding job");
                        }
                    }
                }
            }
